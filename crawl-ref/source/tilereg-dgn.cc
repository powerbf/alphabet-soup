                                    ...
                                    vector<command_type> &cmd)
{
    ...
    {
        str += localize("\n%s %s (%%)", "[Ctrl + L-Click]", "Cast spell");
        cmd.push_back(CMD_CAST_SPELL);
    }

    if (_have_appropriate_evokable(target))
    {
        string key = "[Alt + L-Click]";
#ifdef UNIX
        ...
        if (target && you.can_see(*target))
        {
            ...
            if ((gc - you.pos()).rdist() <= attack_dist)
            {
                if (!cell_is_solid(gc))
                {
                    ...
                    {
                        tip = mon->name(DESC_A);
                        _add_tip(tip, localize("%s %s", "[L-Click]", "Attack"));
                    }
                }
            }
            ...
            if (you.species != SP_FELID
                ...
                && you.m_quiver.get_fire_item() != -1)
            {
                _add_tip(tip, localize("%s %s (%%)", "[Shift + L-Click]", "Fire"));
