...
void DollEditRegion::render()
{
    ...
    m_shape_buf.add(left_gutter + 7, edit_doll_line, left_gutter + 8, edit_doll_line + 1, grey);
    {
        ...
        m_font_buf.add(localize("Custom"), VColour::white, tile_name_x, tile_name_y);
        tile_name_x = (left_gutter + 4.7) * 32.0f;
        tile_name_y = (edit_doll_line + 1) * 32.0f;
        m_font_buf.add(localize("Default"), VColour::white, tile_name_x, tile_name_y);
        tile_name_x = (left_gutter + 7) * 32.0f;
        tile_name_y = (edit_doll_line + 1) * 32.0f;
        m_font_buf.add(localize("Equip"), VColour::white, tile_name_x, tile_name_y);
    }
    ...
    string item_str = localize(part_name);
    ...
    string doll_name;
    doll_name = localize("%s %d / %d", "Doll index", m_doll_idx, NUM_MAX_DOLLS - 1);
    ...
    doll_name = localize("%s: %s", "Doll Mode", mode_name[m_mode]);
    ...
    for (int i = 0 ; i < TILEP_PART_MAX; i++)
    {
        ...
        if (part == TILEP_SHOW_EQUIP)
            info_str = localize("%2s%9s: (%s)", sel, cat_name[i], "show equip");
        else if (!part)
            info_str = localize("%2s%9s: (%s)", sel, cat_name[i], "none");
        else
            info_str = localize("%2s%9s: %3d/%3d", sel, cat_name[i], disp, maxp);
    ...
    // self-explanatory.)
    {
        ...
        m_font_buf.add(
            localize("%-18.18s %-23.23s %-19.19s %s",
        ...
        m_font_buf.add(
            localize("%-18.18s q, %s", "Quit menu", "Ctrl-Q"),
...
void DollEditRegion::run()
{
    ...
    auto title = make_shared<Text>(formatted_string(localize("Doll Editor"), YELLOW));
