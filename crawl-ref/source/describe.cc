...
static string _randart_descrip(const item_def &item)
{
    ...
    for (const property_descriptor &desc : propdescs)
    {
        if (known_proprt(desc.property))
        {
            ...
            {
                sdesc = localise(sdesc, proprt[desc.property]);
            }
...
static string _describe_weapon(const item_def &item, bool verbose)
{
    ...
    if (item_type_known(item) && (spec_ench != SPWPN_NORMAL || enchanted))
    {
        ...
        case SPWPN_FLAMING:
            ...
            {
                description += localise("It causes projectiles fired from it to burn "
                                        "those they strike, "
            ...
            {
                description += localise("It has been specially enchanted to burn "
                                        "those struck by it, "
                                        "causing extra injury to most foes and up to half "
                                        "again as much damage against particularly "
                                        "susceptible opponents.");
        ...
        case SPWPN_FREEZING:
            if (is_range_weapon(item))
            {
                description += localise("It causes projectiles fired from it to freeze "
                                        "those they strike, "
                                        "causing extra injury to most foes and up to half "
                                        "again as much damage against particularly "
                                        "susceptible opponents.");
            ...
            {
                description += localise("It has been specially enchanted to freeze "
                                        "those struck by it, "
                                        "causing extra injury to most foes and up to half "
                                        "again as much damage against particularly "
                                        "susceptible opponents.");
            ...
            if (is_range_weapon(item))
                description += localise(" They can also slow down cold-blooded creatures.");
            else
        ...
        case SPWPN_HOLY_WRATH:
            if (is_range_weapon(item))
            {
                description += localise("It has been blessed by the Shining One"
                                        ", and any projectiles fired from it"
        ...
        case SPWPN_ELECTROCUTION:
            if (is_range_weapon(item))
            {
                description += localise("It charges the ammunition it shoots with "
                    "electricity; occasionally upon a hit, such missiles "
                    "may discharge and cause terrible harm.");
            }
            else
            {
                description += localise("Occasionally, upon striking a foe, it can "
                    "discharge electrical energy and cause terrible harm.");
            }
            break;
        case SPWPN_VENOM:
            if (is_range_weapon(item))
                description += localise("It poisons the ammo it fires.");
            else
        ...
        case SPWPN_VORPAL:
            if (is_range_weapon(item))
            {
                description += localise("Any projectile fired from it inflicts extra damage.");
            }
            else
            {
        ...
        case SPWPN_VAMPIRISM:
            description += localise("It inflicts no extra harm, but heals "
        ...
        case SPWPN_DISTORTION:
            description += localise("It warps and distorts space around it. "
                "Unwielding it can cause banishment or high damage.");
            break;
        case SPWPN_PENETRATION:
            description += localise("Ammo fired by it passes through the "
                "targets it hits, potentially hitting all targets in "
                "its path until it reaches maximum range.");
            break;
        case SPWPN_REAPING:
            description += localise("If a monster killed with it leaves a "
        ...
        case SPWPN_ANTIMAGIC:
            ...
            break;
        case SPWPN_SPECTRAL:
            description += localise("It retains the spirit of the tree from which "
    ...
    {
        description += "\n\n";
        description += localise("It grants immunity to poison.");
    }
    ...
    if (is_artefact(item))
    {
        ...
        {
            description += "\n";
            description += localise("This weapon may have some hidden properties.");
        }
    }
...
static string _warlock_mirror_reflect_desc()
{
    ...
    desc += localise("With your current SH, it has a %d%% chance to reflect "
                     "enchantments and other normally unblockable effects.",
                     reflect_chance);
    return desc;
}
...
static string _describe_armour(const item_def &item, bool verbose)
{
    ...
    if ((ego != SPARM_NORMAL || enchanted) && item_type_known(item) && verbose)
    {
        ...
        case SPARM_RUNNING:
            if (item.sub_type == ARM_BARDING && you.species == SP_NAGA)
                ego_desc = "It allows its wearer to slither at a great speed.";
            else
                ego_desc = "It allows its wearer to run at a great speed.";
            break;
        case SPARM_FIRE_RESISTANCE:
            ego_desc = "It protects its wearer from heat.";
            break;
        case SPARM_COLD_RESISTANCE:
            ego_desc = "It protects its wearer from cold.";
            break;
        case SPARM_POISON_RESISTANCE:
            ego_desc = "It protects its wearer from poison.";
            break;
        case SPARM_SEE_INVISIBLE:
            ego_desc = "It allows its wearer to see invisible things.";
            break;
        case SPARM_INVISIBILITY:
            ego_desc = "When activated it hides its wearer from "
                "the sight of others, but also drains their skills.";
            break;
        case SPARM_STRENGTH:
            ego_desc = "It increases the physical power of its wearer (+3 to strength).";
            break;
        case SPARM_DEXTERITY:
            ego_desc = "It increases the dexterity of its wearer (+3 to dexterity).";
            break;
        case SPARM_INTELLIGENCE:
            ego_desc = "It makes you more clever (+3 to intelligence).";
            break;
        case SPARM_PONDEROUSNESS:
            ego_desc = "It is very cumbersome, thus slowing your movement.";
            break;
        case SPARM_FLYING:
            ego_desc = "It can be activated to allow its wearer to "
                "fly indefinitely.";
            break;
        case SPARM_WILLPOWER:
            ego_desc = "It increases its wearer's willpower, protecting against "
                "some magical effects.";
            break;
        case SPARM_PROTECTION:
            ego_desc = "It protects its wearer from harm (+3 to AC).";
            break;
        case SPARM_STEALTH:
            ego_desc = "It enhances the stealth of its wearer.";
            break;
        case SPARM_RESISTANCE:
            ego_desc = "It protects its wearer from the effects "
                "of both cold and heat.";
            break;
        case SPARM_POSITIVE_ENERGY:
            ego_desc = "It protects its wearer from "
                "the effects of negative energy.";
            break;

        // This is only for robes.
        case SPARM_ARCHMAGI:
            ego_desc = "It increases the power of its wearer's "
                "magical spells.";
            break;
        case SPARM_PRESERVATION:
            ego_desc = "It provides partial protection from all sources of "
                "acid and corrosion.";
            break;

        case SPARM_REFLECTION:
            ego_desc = "It reflects blocked things back in the "
                "direction they came from.";
            break;

        case SPARM_SPIRIT_SHIELD:
            ego_desc = "It shields its wearer from harm at the cost "
                "of magical power.";
            break;

        case SPARM_NORMAL:
            ASSERT(enchanted);
            ego_desc = "It has no special ego (it is not resistant to "
        ...
        case SPARM_ARCHERY:
            ego_desc = "It improves your effectiveness with ranged "
                       "weaponry, such as bows and javelins (Slay+4).";
            break;

        // These are only for scarves.
        case SPARM_REPULSION:
            ego_desc = "It protects its wearer by repelling missiles.";
            break;

#if TAG_MAJOR_VERSION == 34
        case SPARM_CLOUD_IMMUNE:
            ego_desc = "It does nothing special.";
            break;
#endif

        case SPARM_HARM:
            ego_desc = "It increases damage dealt and taken.";
            break;

        case SPARM_SHADOWS:
            ego_desc = "It reduces the wearer's line of sight "
                       "and spell power.";
            break;

        case SPARM_RAMPAGING:
            ego_desc = "Its wearer takes one free step when moving towards enemies.";
            break;
        }

        description += localise(ego_desc);
    }
    ...
    if (is_artefact(item))
    {
        ...
        {
            description += "\n";
            description += localise("This armour may have some hidden properties.");
        }
    }
    else
    {
    ...
    if (is_artefact(item))
    {
            ...
            !item_ident(item, ISFLAG_KNOW_TYPE))
        {
            ...
            if (jewellery_is_amulet(item))
                description += localise("This amulet may have hidden properties.");
            else
                description += localise("This ring may have hidden properties.");
        }
    }
...
void get_feature_desc(const coord_def &pos, describe_info &inf, bool include_extra)
{
    ...
    {
        long_desc += "\n";
        long_desc += localise("(Pray here with '%s' to learn more.)",
    ...
    {
        long_desc += "\n";
        long_desc += localise("It is susceptible to bolts of lightning "
                              "and to sufficiently intense sources of fire.");
    }
...
static string _spell_sources(const spell_type spell)
{
    ...
    if (books.empty())
        return "\n" + localise("This spell is not found in any books.");

    string desc;

    desc += "\n";
    if (books.size() > 1)
        localise("This spell can be found in the following books:");
    else
        localise("This spell can be found in the following book:");
    desc += "\n ";
...
static string _flavour_base_desc(attack_flavour flavour)
{
    static const map<attack_flavour, string> base_descs = {
        ...
        { AF_BLINK,             " and blink itself if any damage is dealt" },
        ...
        { AF_ENGULF,            " and engulf with water" },
        { AF_PURE_FIRE,         "" },
...
static string _monster_missiles_description(const monster_info& mi)
{
    ...
    string name = missile->name(DESC_A, false, false, true, ISFLAG_KNOW_CURSE);
    string desc = localise("It is quivering %s.", name);
    desc += "\n";
    return desc;
}
                         ...
                         bool &has_stat_desc)
{
    ...
    if (mi.is(MB_SUMMONED) || mi.is(MB_PERM_SUMMON))
    {
        ...
        if (!did_stair_use && !mi.is(MB_PERM_SUMMON))
            inf.body << localise(" and it is incapable of using stairs");
