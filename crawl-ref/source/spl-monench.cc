...
int englaciate(coord_def where, int pow, actor *agent)
{
        ...
        || victim->is_stationary())
    {
        if (!mons)
            ...
            simple_monster_message(*mons, "%s is unaffected.");
        return 0;
    }
...
bool backlight_monster(monster* mons)
{
    ...
    if (lvl == 0)
        simple_monster_message(*mons, "%s is outlined in light.");
    else if (lvl == 4)
        simple_monster_message(*mons, "%s glows brighter for a moment.");
    else
        simple_monster_message(*mons, "%s glows brighter.");
...
bool do_slow_monster(monster& mon, const actor* agent, int dur)
{
        ...
        && mon.add_ench(mon_enchant(ENCH_SLOW, 0, agent, dur)))
    {
        if (!mon.paralysed() && !mon.petrified()
            && simple_monster_message(mon, "%s seems to slow down."))
        {
            return true;
        }
    }
