...
static string _describe_ash_skill_boost()
{
    ...
    {
        return localise("Ashenzari won't support your skills until you bind "
                        "yourself with cursed items.");
    }
    ...
    desc.setf(ios::left);
    desc << "<white>";
    desc << chop_string(localise("Bound part"), 18);
    desc << chop_string(localise("Boosted skills"), 30);
    desc << localise("Bonus") << "\n";
    desc << "</white>";
    ...
    for (int i = ET_WEAPON; i < NUM_ET; i++)
    {
        ...
        if (i == ET_WEAPON && you.bondage[i] == 3)
            desc << chop_string(localise("Hands"), 18);
        else
            desc << chop_string(localise(bondage_parts[i]), 18);
        ...
        while (!boosted_skills.empty())
        {
            ...
            if (it->first == SK_CONJURATIONS)
                skills += localise("Magic schools");
            else
                skills += localise(skill_name(it->first));
        ...
        desc << chop_string(skills, 30);
        desc << localise(bonus_level[bonus -1]) << "\n";
    }
...
static formatted_string _god_extra_description(god_type which_god)
{
    ...
    switch (which_god)
    {
        case GOD_ASHENZARI:
            ...
            {
                desc.cprintf("\n");
                _add_par(desc, localise("Ashenzari supports the following "
                                        "skills because of your curses:"));
...
static formatted_string _describe_god_powers(god_type which_god)
{
    ...
    case GOD_ASHENZARI:
        have_any = true;
        desc.cprintf(localise("You are provided with a bounty of information.")
                     + "\n");
        break;
    ...
    {
        have_any = true;
        desc.cprintf(localise("%s prevents your magic from regenerating.",
                              god_name(which_god))
                     + "\n");
        desc.cprintf(localise("%s identifies device charges for you.",
                              god_name(which_god))
                     + "\n");
...
static void _send_god_ui(god_type god, bool is_altar)
{
    ...
    if (you_worship(god))
    {
        tiles.json_write_string("title", god_title(god, you.species, you.piety, false));
