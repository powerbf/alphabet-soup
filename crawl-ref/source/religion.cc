...
void god_power::display(bool gaining, const char* fmt) const
{
    ...
    if (isupper(str[0]))
        mpr(MSGCH_GOD, god, str);
    else
        mprf(MSGCH_GOD, god, fmt, str);
}
        ...
        && fedhas_protects(target))
    {
        if (!quiet && you.can_see(*target))
        {
            if (agent->is_player())
                ...
                simple_god_message(" protects a plant from harm.", GOD_FEDHAS);
        }
        return true;
    }
...
void god_pitch(god_type which_god)
{
    ...
    else
    {
        ...
        replace_first(msg, "%s", get_form()->player_prayer_action());
