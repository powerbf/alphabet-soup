...
void monster::remove_enchantment_effect(const mon_enchant &me, bool quiet)
{
    ...
    case ENCH_HASTE:
        calc_speed();
        if (!quiet)
            simple_monster_message(*this, "%s is no longer moving quickly.");
        break;
    ...
    case ENCH_SWIFT:
        if (!quiet)
        {
            if (type == MONS_ALLIGATOR)
                simple_monster_message(*this, "%s slows down.");
            else
                simple_monster_message(*this, "%s is no longer moving quickly.");
        }
        break;
    ...
    case ENCH_SILENCE:
        ...
        {
            if (alive())
                simple_monster_message(*this, "%s becomes audible again.");
            else
    ...
    case ENCH_MIGHT:
        if (!quiet)
            simple_monster_message(*this, "%s no longer looks unusually strong.");
        break;

    case ENCH_SLOW:
        if (!quiet)
            simple_monster_message(*this, "%s is no longer moving slowly.");
        calc_speed();
        break;
    ...
    case ENCH_PARALYSIS:
        if (!quiet)
            simple_monster_message(*this, "%s is no longer paralysed.");
    ...
    case ENCH_PETRIFIED:
        if (!quiet)
            simple_monster_message(*this, "%s is no longer petrified.");
    ...
    case ENCH_CONFUSION:
        if (!quiet)
            simple_monster_message(*this, "%s seems less confused.");
    ...
    if (!quiet)
        {
            if (visible_to(&you))
                simple_monster_message(*this, "%s stops glowing.");
    ...
    case ENCH_STICKY_FLAME:
        if (!quiet)
            simple_monster_message(*this, "%s stops burning.");
        break;

    case ENCH_POISON:
        if (!quiet)
            simple_monster_message(*this, "%s looks more healthy.");
        break;
    ...
    case ENCH_HELD:
    {
        ...
        if (net != NON_ITEM)
        {
            ...
            if (!quiet)
                simple_monster_message(*this, "%s breaks free.");
            break;
        }
    ...
    case ENCH_ABJ:
        ...
        if (berserk())
            simple_monster_message(*this, "%s is no longer berserk.");
    ...
    case ENCH_LIQUEFYING:
        ...
        if (!quiet)
            simple_monster_message(*this, "%s is no longer liquefying the ground.");
        break;
    ...
    case ENCH_DAZED:
        if (!quiet && alive())
                simple_monster_message(*this, "%s is no longer dazed.");
        break;

    case ENCH_INNER_FLAME:
        if (!quiet && alive())
            simple_monster_message(*this, "%s's inner flame fades away.");
        break;
    ...
    case ENCH_MUTE:
        if (!quiet && alive())
                simple_monster_message(*this, "%s is no longer mute.");
        break;

    case ENCH_BLIND:
        if (!quiet && alive())
            simple_monster_message(*this, "%s is no longer blind.");
    ...
    case ENCH_DUMB:
        if (!quiet && alive())
            simple_monster_message(*this, "%s is no longer stupefied.");
    ...
    case ENCH_MAD:
        if (!quiet && alive())
            simple_monster_message(*this, "%s is no longer mad.");
    ...
    case ENCH_REGENERATION:
        if (!quiet)
            simple_monster_message(*this, "%s is no longer regenerating.");
        break;

    case ENCH_STRONG_WILLED:
        if (!quiet)
            simple_monster_message(*this, "%s is no longer strong-willed.");
        break;
    ...
    case ENCH_WEAK:
        if (!quiet)
            simple_monster_message(*this, "%s is no longer weakened.");
        break;
    ...
    case ENCH_FIRE_VULN:
        if (!quiet)
            simple_monster_message(*this, "%s is no longer more vulnerable to fire.");
        break;
    ...
    case ENCH_POISON_VULN:
        if (!quiet)
            simple_monster_message(*this, "%s is no longer more vulnerable to poison.");
        break;
    ...
    case ENCH_AGILE:
        if (!quiet)
            simple_monster_message(*this, "%s is no longer unusually agile.");
        break;

    case ENCH_FROZEN:
        if (!quiet)
            simple_monster_message(*this, "%s is no longer encased in ice.");
        calc_speed();
        break;
    ...
    case ENCH_BLACK_MARK:
        if (!quiet)
        {
            simple_monster_message(*this, "%s is no longer absorbing vital"
                                         " energies.");
        }
        calc_speed();
        break;
    ...
    case ENCH_SAP_MAGIC:
        if (!quiet)
            simple_monster_message(*this, "%s is no longer being sapped.");
        break;

    case ENCH_CORROSION:
        if (!quiet)
           simple_monster_message(*this, "%s is no longer covered in acid.");
        break;

    case ENCH_GOLD_LUST:
        if (!quiet)
           simple_monster_message(*this, "%s is no longer distracted by gold.");
        break;

    case ENCH_DRAINED:
        if (!quiet)
            simple_monster_message(*this, "%s seems less drained.");
        break;

    case ENCH_REPEL_MISSILES:
        if (!quiet)
            simple_monster_message(*this, "%s is no longer repelling missiles.");
        break;

    case ENCH_RESISTANCE:
        if (!quiet)
            simple_monster_message(*this, "%s is no longer unusually resistant.");
        break;

    case ENCH_BRILLIANCE_AURA:
        if (!quiet)
            simple_monster_message(*this, "%s is no longer giving off an aura.");
        break;

    case ENCH_EMPOWERED_SPELLS:
        if (!quiet)
            simple_monster_message(*this, "%s seems less brilliant.");
        break;

    case ENCH_IDEALISED:
        if (!quiet)
            simple_monster_message(*this, "%s loses the glow of perfection.");
        break;
    ...
    case ENCH_INFESTATION:
        if (!quiet)
            simple_monster_message(*this, "%s is no longer infested.");
        break;
    ...
    case ENCH_WATERLOGGED:
        if (!quiet)
            simple_monster_message(*this, "%s is no longer waterlogged.");
        break;

    case ENCH_ROLLING:
        if (!quiet)
            simple_monster_message(*this, "%s stops rolling and uncurls.");
        break;
...
void monster::apply_enchantment(const mon_enchant &me)
{
    ...
    case ENCH_INSANE:
        ...
        {
            simple_monster_message(*this, "%s is no longer in an insane frenzy.");
    ...
    case ENCH_FATIGUE:
        if (decay_enchantment(en))
        {
            simple_monster_message(*this, "%s looks more energetic.");
    ...
    case ENCH_MIRROR_DAMAGE:
        if (decay_enchantment(en))
            simple_monster_message(*this, "%s's dark mirror aura disappears.");
        break;
    ...
    case ENCH_STICKY_FLAME:
    {
        ...
        if (dam > 0)
        {
            simple_monster_message(*this, "%s burns!");
    ...
    case ENCH_PORTAL_PACIFIED:
    {
        if (decay_enchantment(en))
        {
            ...
            {
                if (you.can_see(*this))
                    simple_monster_message(*this, "%s suddenly becomes enraged!");
                else
    ...
    case ENCH_SEVERED:
    {
        simple_monster_message(*this, "%s writhes!");
        coord_def base_position = props["base_position"].get_coord();
        maybe_bloodify_square(base_position);
        hurt(me.agent(), 20);
    }
    ...
    case ENCH_SILVER_CORONA:
        if (how_chaotic())
        {
            ...
            simple_monster_message(*this, "%s is seared!");
